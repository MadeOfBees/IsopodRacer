...
#func set_active_rope_id(value:int):
		...
		#else:
			#for i in len(rope_segs):
				#if i == active_rope_id:
					#(rope_segs[i] as RigidBody2D).mass = 10
				#else:
					#(rope_segs[i] as RigidBody2D).mass = 1
#
#
...
#func create_rope(segs_amount:int, parent:Object, end_pos:Vector2, spawn_angle:float) -> void:
	...
	#rope_end_joint.node_b = rope_segs[-1].get_path()
	#
#
